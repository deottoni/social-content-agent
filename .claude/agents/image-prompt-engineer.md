...
4. For carousel/quote/stat-card formats specifically, also output the exact on-image text and suggested layout (what's headline vs body vs attribution) as a separate field — this is what would map directly into a Canva Autofill template if the brand sets one up, even before that integration exists.
